...
export class ApiService {
  async getMultipleCharacters(id: number[]): Promise<Character[] | Character> {
    ...
    if (character.status === 200) {
      ...
      if (id.length === 1) {
        throw new Error(
          `Failed to fetch character ${id} with status message: ${character.statusMessage}`,
        );
      } else {
        throw new Error(
          `Failed to fetch characters ${id.join(', ')} with status message: ${character.statusMessage}`,
        );
      }
    }
  }
  ...
  async getEpisode(id: number): Promise<Episode> {
    ...
    if (episode.status === 200) {
      ...
      throw new Error(
        `Failed to fetch episode ${id} with status message: ${episode.statusMessage}`,
      );
    }
  }
  ...
  async getLocation(id: number): Promise<Location> {
    ...
    if (location.status === 200) {
      ...
      throw new Error(
        `Failed to fetch location ${id} with status message: ${location.statusMessage}`,
      );
    }
  }
  ...
  async getCharacters(
    ...
    if (characters.status === 200 || !characters.data.info) {
      ...
      throw new Error(
        `Failed to fetch characters with status message: ${characters.statusMessage}`,
      );
    }
  }
  ...
  async getCharactersInDimension(
    dimension: string,
  ): Promise<{ residents: string[]; locations: Location[] }> {
    const locations = await getLocations({ dimension });
    if (locations.status !== 200 || !locations.data.info) {
      throw new Error(
        `Failed to fetch characters in dimension ${dimension} with status message: ${locations.statusMessage}`,
      );
    }
    ...
    for (let page = 2; page <= totalPages; page++) {
      ...
      if (moreLocations.status === 200 && moreLocations.data) {
        ...
        throw new Error(
          `Failed to fetch characters in dimension ${dimension} with status message: ${moreLocations.statusMessage}`,
        );
      }
    }
    return {
  ...
  async getAllDimensions(): Promise<{ name: string; id: number }[]> {
    ...
    if (locations.status !== 200 || !locations.data.info) {
      throw new Error(
        `Failed to fetch all dimensions with status message: ${locations.statusMessage}`,
      );
    }
    ...
    for (let page = 2; page <= totalPages; page++) {
      ...
      if (moreLocations.status === 200 && moreLocations.data) {
        ...
        throw new Error(
          `Failed to fetch dimensions with status message: ${moreLocations.statusMessage}`,
        );
      }
    }
    return results;
