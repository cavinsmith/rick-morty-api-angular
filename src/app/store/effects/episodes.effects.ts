...
export class EpisodesEffects {
  ...
  loadEpisodes$ = createEffect(() => this.actions$.pipe(
      ...
      mergeMap(([{ id }, episodes]) => {
        ...
        return episode ? EMPTY : from(this.apiService.getEpisode(id)).pipe(
          map((result) => {
            return EpisodeActions.loadEpisodeSuccess({
              episode: result
            });
          }),
          catchError((error) => of(EpisodeActions.loadEpisodeFailure({ error: error.message })))
        )
    })
  ));
}