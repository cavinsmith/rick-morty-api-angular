...
export class Search<T extends { id: number; [key: string]: any }, F> implements OnInit {
  ...
  @Input() searchName: string = 'unknown';
  ...
  private updatePage() {
    ...
    this.items$ = this.pagesFacade.getPage(1, this.searchFilter)
      .pipe(
        map(entities => {
          if (!entities) return [];
          ...
          const uniqueValues = validValues.filter((item, index, array) => 
            index === array.findIndex(t => t.value === item.value)
          );
